    ...
    <c:otherwise>
     <c:if test="${(isLocked == null) || (isLocked eq 'no')}">
      <c:set var="imageFileName" value="icon_noNote" />
       ...
       <a tabindex="<c:out value="${tabNum + 1000}"/>" href="#"  onmouseover="callTip(genToolTips(${itemId}));"
           onmouseout="UnTip();" onClick=
    "openDNWindow('CreateDiscrepancyNote?rowCount=${fn:escapeXml(param.rowCount)}&eventCRFId=${fn:escapeXml(eventCRFId)}&isGroup=1&subjectId=<c:out value="${studySubject.id}" />&itemId=<c:out value="${itemId}" />&groupLabel=' + encodeURIComponent('<c:out value="${displayItem.metadata.groupLabel}"/>') + '&sectionId=<c:out value="${displayItem.metadata.sectionId}"/>&id=<c:out value="${item_data_id}"/>&name=itemData&field=<c:out value="${inputName}"/>&column=value&monitor=1&errorFlag=<c:out value="${errorFlag}"/>&isLocked=<c:out value="${isLocked}"/>&eventName='+ encodeURIComponent('<c:out value="${eventName}"/>') +'&eventDate=${fn:escapeXml(eventDate)}&crfName='+ encodeURIComponent('<c:out value="${crfName}"/>'),'spanAlert-<c:out value="${inputName}"/>','<c:out value="${errorTxtMessage}"/>'); return false;"
